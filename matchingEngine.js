// matchingEngine.js — Real Prop Firm Execution Engine (PRODUCTION-READY UPGRADE)
...
const { markSymbolActive, markSymbolInactive } = require("./index");
...
async function processTick(symbol, price) {
  ...
  const toClose = openTrades.filter(t => {
    ...
    if (t.side === "buy") {
      return (t.stop_loss != null && price <= t.stop_loss) || (t.take_profit != null && price >= t.take_profit);
    } else {
      return (t.stop_loss != null && price >= t.stop_loss) || (t.take_profit != null && price <= t.take_profit);
    }
  });
...
async function preTradeRiskCheck(order, account) {
  ...
  if (contract.maxLots?.[account.account_type] && order.size > contract.maxLots[account.account_type])
    return { ok: false, reason: "MAX_LOT_SIZE" };

  if (await breachesLossLimits(account))
...
async function placeOrder(order) {
  ...
  if (order.type === "market") {
    ...
    // ✅ Double-check price is within allowed slippage
...
async function fillOrder(order, basePrice) {
  // Prevent race conditions: one order fills at a time per account
  const lock = accountLocks.get(order.account_id) || Promise.resolve();
  accountLocks.set(order.account_id, lock.then(async () => {
    setTimeout(async () => {
      const contract = getContracts()[order.symbol];
      ...
      if (order.id) {
        await supabase.from("orders").update({ status: "filled", entry_price: execPrice, filled_at: new Date().toISOString() }).eq("id", order.id);
      }
...
async function closeTrade(trade, closePrice) {
  ...
  const stillActive = pendingOrders.some(o => o.symbol === trade.symbol) || openTrades.some(t => t.symbol === trade.symbol);
...
async function checkMIL(acc, price) {
  if (await breachesLossLimits(acc)) {
    ...
    await supabase.from("accounts").update({ status: "blown", breach_reason: "MAX_INTRADAY_LOSS" }).eq("id", acc.id);
